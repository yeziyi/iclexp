...
import com.sagittarius.icleanexpert.R;
...
public class CleanMasterAdapter extends BaseExpandableListAdapter {
			...
			if (groupPosition == 0) {
				...
				if (bs) {
					...
					RamAppDialog dialog = new RamAppDialog(
							...
							}, null, TAApplication.getApplication().getString(
									R.string.sysapptitle), TAApplication
				...
				if (!AppUtils.isSystemApp(TAApplication.getApplication(),
					...
					if (!ram.isSelect) {
						...
						RamAppDialog dialog = new RamAppDialog(
								...
								}, null, TAApplication.getApplication()
										.getString(R.string.sysapptitle),
	...
	public View getGroupView(int groupPosition, boolean isExpanded,
		...
		String result = String.format(
				TAApplication.getApplication()
						.getString(R.string.listitemtitle), (selectCount + ""),
				sizeStr);
		int start = result.indexOf(sizeStr);
		int end = start + sizeStr.length();
		Spannable word = new SpannableString(result);
		word.setSpan(new ForegroundColorSpan(0xFFea7d00), start, end,
		...
		if (getChildrenCount(groupPosition) == 0) {
			arrow.setImageResource(R.drawable.arrow_down);
			info1.setText(TAApplication.getApplication().getString(
					R.string.listitemtitlenothing));
			convertView.findViewById(R.id.gap).setVisibility(View.VISIBLE);
		}
		return convertView;
	}
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		if (groupPosition == 0) {
			...
			info1.setText(TAApplication.getApplication().getString(
					R.string.ramrealease));
			info2.setText(ram.ram_str);
			if (AppUtils.isSystemApp(TAApplication.getApplication(),
					ram.pkgName)) {
				tinfo.setVisibility(View.VISIBLE);
				tinfo.setText(TAApplication.getApplication().getString(
						R.string.systemappram));
			}
		} else if (groupPosition == 3) {
			...
			info12.setText(TAApplication.getApplication().getString(
					R.string.bigfilesize));
	...
	public void update(CleanMasterDataBean bean) {
		...
		if (tList1.size() > 0) {
			...
			tcb.isSelect = true;
			tcb.name = String.format(TAApplication.getApplication()
					.getResources().getString(R.string.thumbnails),
					(tList1.size() + ""));
			long size = 0;
		...
		if (tList2.size() > 0) {
			...
			tcb.isSelect = true;
			tcb.name = String.format(TAApplication.getApplication()
					.getResources().getString(R.string.emptydir),
					(tList2.size() + ""));
			long size = 0;
		...
		if (tList3.size() > 0) {
			...
			tcb.isSelect = true;
			tcb.name = String.format(TAApplication.getApplication()
					.getResources().getString(R.string.tmpfile),
					(tList3.size() + ""));
			long size = 0;
		...
		if (tList4.size() > 0) {
			...
			tcb.isSelect = true;
			tcb.name = String.format(TAApplication.getApplication()
					.getResources().getString(R.string.logfile),
					(tList4.size() + ""));
			long size = 0;
		...
		if (tList5.size() > 0) {
			...
			tcb.isSelect = true;
			tcb.name = String.format(TAApplication.getApplication()
					.getResources().getString(R.string.emptyfile),
					(tList5.size() + ""));
			long size = 0;
